import {insertJevExplorer} from './jev-explorer.mjs';
import {insertContextExplorer} from './context-explorer.mjs';
import {socialImageFor,socialImageTags} from './social-image.mjs';
...
function layout(c,{title,description,body,route='',preview=false,article=null,socialPackage=null,toc=[]}) {
  const base=c.basePath, url=article?canonical(c,article.slug):`${canonical(c)}${route}`;
  const social=socialImageFor(c,article||socialPackage);
...
function articlePage(c,p,preview) {
  ...
  if(p.slug==='keeping-parallel-development-fast') {
    const anchor='<h2 id="docker-has-two-different-cache-mechanisms">';
    rendered.html=insertDevops(rendered.html,anchor);
    const at=rendered.toc.findIndex(h=>h.id==='docker-has-two-different-cache-mechanisms');
    rendered.toc.splice(at,0,{id:'devops-title',text:'Interactive: follow the change'});
  }
...
async function buildUnlocked(root,{preview=false,outDir='dist',force=false}={}) {
  ...
  const visible=all.filter(p=>p.meta.status==='published'||(preview&&p.meta.status!=='archived')).sort((a,b)=>b.meta.createdAt.localeCompare(a.meta.createdAt)||(b.meta.seriesOrder||0)-(a.meta.seriesOrder||0));
  ...
  if (visible.some(p => p.slug === 'keeping-parallel-development-fast') && !visible.some(p => p.slug === 'parallel-agent-engineering')) {
    for (const file of ['devops-flow.css','devops-flow.js','devops-model.js'])
      await put(file,await readFile(path.join(root,'site',file)));
  }
  if(visible.some(p=>p.slug==='better-context-for-ai-agents')) {
    for(const file of ['context-explorer.css','context-explorer.js','context-model.js'])
      await put(file,await readFile(path.join(root,'site',file)));
  }
  if(visible.some(p=>p.slug==='jev-decision-layer')){
    for(const file of ['jev-model.js','jev-explorer.js','jev-explorer.css'])await put(file,await readFile(path.join(root,'site',file)));
  }
  await put('.nojekyll','');
